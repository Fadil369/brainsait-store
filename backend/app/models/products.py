...
class Product(Base):
    ...
    compare_price = Column(Numeric(10, 2), nullable=True)  # Original price for discount display
    cost_price = Column(Numeric(10, 2), nullable=True)  # Cost for margin calculation
    
    # Tax
    tax_rate = Column(Numeric(5, 2), nullable=False, default=15.00)  # 15% VAT for Saudi Arabia
    ...
    stock_status = Column(Enum(StockStatus), nullable=False, default=StockStatus.IN_STOCK)
    ...
    def discount_percentage(self) -> Optional[float]:
        ...
        if not self.is_on_sale:
            return None
        return ((float(self.compare_price) - float(self.price)) / float(self.compare_price)) * 100
