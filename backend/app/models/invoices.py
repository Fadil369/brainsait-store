...
class Invoice(Base):
    ...
    invoice_type = Column(Enum(InvoiceType), nullable=False, default=InvoiceType.TAX_INVOICE)
    status = Column(Enum(InvoiceStatus), nullable=False, default=InvoiceStatus.DRAFT)
    
    # ZATCA Compliance
    zatca_status = Column(Enum(ZATCAStatus), nullable=False, default=ZATCAStatus.PENDING)
...
class InvoiceLineItem(Base):
    ...
    unit_of_measure = Column(String(20), nullable=False, default="PCE")  # Piece, KG, etc.
    ...
    tax_amount = Column(Numeric(15, 2), nullable=False)
    tax_category = Column(String(50), nullable=False, default="S")  # S=Standard, Z=Zero, E=Exempt
...
class InvoiceSequence(Base):
    ...
    # Indexes
    __table_args__ = (
        Index('idx_invoice_sequences_tenant_prefix', 'tenant_id', 'prefix', 'year', 'month', unique=True),
    )
    ...
    def generate_number(self) -> str:
        ...
        if self.year and self.month:
            ...
            return f"{self.prefix}-{self.year:04d}-{self.current_number:0{self.padding}d}"
        else:
