...
from app.models.oid import OIDNode, HealthcareProvider
from app.models.store import Product, Order
from app.schemas.oid import (
    OIDNodeResponse, OIDTreeResponse, HealthcareProviderResponse,
    OIDNodeCreate, OIDNodeUpdate, HealthcareProviderCreate,
    OIDIntegrationMetrics, OIDSyncStatus
)
    ...
    try:
        ...
        for node in nodes:
            node_data = {
                ...
                "description": node.description_ar if lang == "ar" and node.description_ar else node.description,
                "node_type": node.node_type,
                "status": node.status,
                "metadata": node.metadata or {},
                "neural_capabilities": node.neural_capabilities or [],
                "performance_metrics": node.performance_metrics or {} if include_metrics else {},
                "created_at": node.created_at,
                "updated_at": node.updated_at
            }
            
            # Add children if any
            children_query = select(OIDNode).where(
                OIDNode.parent_id == node.id
            )
            ...
            node_data["children"] = [
                {
                    ...
                    "name": child.name_ar if lang == "ar" and child.name_ar else child.name,
        ...
        healthcare_nodes = len([n for n in nodes if n.node_type == "healthcare_provider"])
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to fetch OID tree"
        )
...
async def create_oid_node(
    ...
    try:
        ...
        if existing_result.scalar_first():
            raise HTTPException(
                status_code=400,
                detail="OID path already exists"
            )
        ...
        if node_data.parent_id:
            parent_query = select(OIDNode).where(
                and_(
                    OIDNode.id == node_data.parent_id,
                    OIDNode.tenant_id == tenant_id
                )
            )
        ...
        if node.node_type == "healthcare_provider":
            background_tasks.add_task(
                register_healthcare_provider_nphies,
                node.id
            )
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to create OID node"
        )
...
async def link_product_to_oid_node(
    ...
    try:
        ...
        background_tasks.add_task(
            sync_product_oid_link_to_obsidian,
            product_id,
            node_id
        )
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to link product to OID node"
        )
...
async def get_healthcare_providers(
    ...
    query = select(HealthcareProvider).join(OIDNode).where(
        HealthcareProvider.tenant_id == tenant_id
    )
    ...
    return [
        HealthcareProviderResponse(
            ...
            provider_name=provider.provider_name_ar if lang == "ar" and provider.provider_name_ar else provider.provider_name,
...
async def create_healthcare_provider(
    ...
    try:
        ...
        oid_path = generate_healthcare_provider_oid(tenant_id, provider_data.license_number)
        ...
        background_tasks.add_task(
            register_provider_with_nphies,
            provider.id
        )
        
        # Sync to Obsidian
        background_tasks.add_task(
            sync_healthcare_provider_to_obsidian,
            provider.id
        )
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to create healthcare provider"
        )
...
async def get_oid_integration_metrics(
    ...
    try:
        ...
        nodes_query = select(
            ...
            func.sum(func.case([(OIDNode.status == 'active', 1)], else_=0)).label('active_nodes'),
            func.sum(func.case([(OIDNode.node_type == 'healthcare_provider', 1)], else_=0)).label('healthcare_nodes')
        ...
        products_query = select(
            ...
            func.sum(func.case([(func.json_extract(Product.metadata, '$.oid_node_id').isnot(None), 1)], else_=0)).label('linked_products')
        ...
        healthcare_orders_query = select(
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to fetch integration metrics"
        )
...
def calculate_integration_health_score(active_nodes: int, total_nodes: int, linked_products: int, total_products: int) -> float:
...
async def sync_healthcare_provider_to_obsidian(provider_id: UUID):
    ...
    try:
        ...
        logger.error(f"Failed to sync healthcare provider {provider_id} to Obsidian: {e}")
