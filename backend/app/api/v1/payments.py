...
from app.models.store import Order, Payment, Invoice
from app.models.products import Product
from app.schemas.payments import (
    PaymentIntentCreate, PaymentIntentResponse,
    PaymentWebhook, MadaPaymentCreate, STCPaymentCreate,
    InvoiceResponse, PaymentMethodResponse, PayPalPaymentCreate,
    ApplePayPaymentCreate, StripeProductCreate, PaymentLinkCreate
)
from app.services.payment_providers import StripeService, MadaService, STCPayService
...
async def create_stripe_payment_intent(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Payment intent creation failed"
        )
...
async def create_mada_payment_intent(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Mada payment creation failed"
        )
...
async def create_stc_payment_intent(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="STC Pay payment creation failed"
        )
...
async def validate_apple_pay_merchant(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Apple Pay validation failed"
        )
...
async def process_apple_pay_payment(
    ...
    try:
        ...
        payment = Payment(
            ...
            currency="SAR",
            status="pending",
            metadata={"apple_pay": True, "client_secret": payment_intent["client_secret"]}
        )
        ...
        raise HTTPException(
            status_code=500,
            detail="Apple Pay payment failed"
        )
            ...
            try:
                ...
                stripe_price = stripe.Price.create(
                    ...
                    metadata={
                        ...
                        "includes_source": str("source_code" in (product.bundle_options or []))
                    }
                )
...
async def create_stripe_payment_link(
    ...
    try:
        ...
        products_query = select(Product).where(
            Product.id.in_(link_data.product_ids)
        )
        products_result = await db.execute(products_query)
        products = products_result.scalars().all()
    ...
    try:
        ...
        if webhook_data["event_type"] == "CHECKOUT.ORDER.APPROVED":
            ...
            await handle_payment_success(
                db, background_tasks,
                provider="paypal",
                provider_payment_id=capture_data["supplementary_data"]["related_ids"]["order_id"],
            ...
            await handle_payment_failure(
                ...
                provider="paypal",
                provider_payment_id=capture_data["supplementary_data"]["related_ids"]["order_id"],
                error={"message": "Payment capture denied", "reason": capture_data.get("status_details", {})}
            )
...
async def handle_payment_success(
    ...
    try:
        ...
        if not payment:
            logger.error(f"Payment not found for {provider} payment {provider_payment_id}")
            return
...
async def send_payment_failure_notification(email: str, order_id: UUID, error_message: str):
