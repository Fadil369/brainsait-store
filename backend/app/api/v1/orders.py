...
from app.core.dependencies import (
    ...
    get_tenant_id, get_language, verify_rate_limit
)
from app.core.localization import get_localized_message
from app.models.users import User
from app.models.products import Product, ProductVariant
from app.models.orders import (
    CartItem, Order, OrderItem, OrderStatusHistory, Coupon
)
from app.schemas.orders import (
    CartItemCreate, CartItemUpdate, CartItemResponse, CartSummary,
    OrderCreate, OrderUpdate, OrderResponse, OrderListResponse,
    OrderStatusHistoryResponse, CheckoutRequest, CheckoutResponse,
...
async def get_cart(
    ...
    for cart_item in cart_items:
        ...
        unit_price = float(variant.price) if variant and variant.price else float(product.price)
...
async def list_orders(
    ...
    for order in orders:
        # Get item count
        item_count_query = select(func.count(OrderItem.id)).where(OrderItem.order_id == order.id)
...
async def checkout(
    ...
    for cart_item in cart_items:
        ...
        order_items.append({
            ...
            'tax_rate': tax_rate,
            'tax_amount': tax_amount,
            'product_image': product.images[0] if product.images else (variant.image_url if variant else None)
        })
    ...
    order = Order(
        ...
        customer_email=checkout_data.billing_address.first_name + " " + checkout_data.billing_address.last_name,
        customer_phone=checkout_data.billing_address.phone,
        customer_name=checkout_data.billing_address.first_name + " " + checkout_data.billing_address.last_name,
