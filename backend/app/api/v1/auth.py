...
from app.models.users import LoginSession, User, UserRole
...
async def register(
    ...
    existing_user = (
        db.query(User)
        .filter(User.email == user_data.email, User.tenant_id == tenant.id)
        .first()
    )
...
async def oauth_authorize(
    ...
    authorization_url = f"{sso_config.authorization_url}?" + urllib.parse.urlencode(
        auth_params
    )
...
async def oauth_callback(
    ...
    sso_config = (
        db.query(TenantSSO)
        .filter(TenantSSO.tenant_id == tenant.id, TenantSSO.provider == provider)
        .first()
    )
...
async def saml_assertion_consumer(
    ...
    if not saml_response:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Missing SAML response"
        )
...
async def authenticate_active_directory(
    ...
    user_info = await authenticate_ldap(
        ad_config, credentials["username"], credentials["password"]
    )
...
async def get_sso_providers(
    ...
    providers = (
        db.query(TenantSSO)
        .filter(TenantSSO.tenant_id == tenant.id, TenantSSO.is_active == True)
        .all()
    )
    ...
    return [
        {
            ...
            "login_url": (
                f"/auth/{p.sso_type}/{p.provider}/authorize"
                if p.sso_type == "oauth"
                else f"/auth/saml/{p.provider}/sso"
            ),
        }
        for p in providers
    ]
...
async def get_oauth_user_info(
    sso_config: TenantSSO, access_token: str
) -> Dict[str, Any]:
...
async def authenticate_ldap(
    ad_config: TenantSSO, username: str, password: str
...
async def find_or_create_oauth_user(
    db: Session, tenant: Tenant, provider: str, user_info: Dict[str, Any]
) -> User:
...
async def find_or_create_saml_user(
    db: Session, tenant: Tenant, provider: str, attributes: Dict[str, Any]
) -> User:
...
async def find_or_create_ad_user(
    db: Session, tenant: Tenant, user_info: Dict[str, Any]
) -> User:
