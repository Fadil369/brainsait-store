...
from app.schemas.payment import (
    PaymentRequest, PaymentResponse, StripeProductCreate,
    StripeSubscriptionCreate, PayPalPaymentCreate, ApplePayValidation
)
...
class PaymentGatewayService:
    ...
    async def create_stripe_product(self, product_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def create_stripe_payment_link(self, price_id: str, metadata: Dict[str, Any] = None) -> str:
        ...
        try:
            ...
            payment_intent = self.stripe.PaymentIntent.create(
                ...
                payment_method_data={
                    "type": "card",
                    "card": {
                        "token": payment_token["paymentData"]["data"]
                    }
                },
                metadata={
                    "order_id": str(order.id),
        ...
        try:
            if payment_method == "stripe":
                return await self.create_stripe_checkout_session(
                    order,
                    payment_data.get("success_url", f"{settings.FRONTEND_URL}/payment/success"),
                    payment_data.get("cancel_url", f"{settings.FRONTEND_URL}/payment/cancel")
                )
