...
class AnalyticsService:
    ...
    async def get_revenue_analytics(
        ...
        ).filter(*filters).scalar() or Decimal('0.00')
        
        # Revenue by payment method
        revenue_by_payment = self.db.query(
            Order.payment_method,
            func.sum(Order.total_amount).label('revenue'),
            func.count(Order.id).label('order_count')
        ).filter(*filters).group_by(Order.payment_method).all()
        ...
        prev_revenue = self.db.query(
            func.sum(Order.total_amount)
        ).filter(*prev_filters).scalar() or Decimal('0.00')
    ...
    async def get_customer_analytics(
        ...
        # Total customers
        total_customers = self.db.query(func.count(User.id)).filter(*base_filters).scalar()
        ...
        customer_acquisition = self.db.query(
    ...
    async def get_product_analytics(
        ...
        category_performance = self.db.query(
            ...
            *order_filters
        ).group_by(Product.category).order_by(desc('total_revenue')).all()
    ...
    async def get_payment_analytics(
        ...
        payment_distribution = self.db.query(
            Order.payment_method,
        ...
        success_rates = self.db.query(
            Order.payment_method,
            func.count(case([(Order.status == OrderStatus.COMPLETED, 1)])).label('successful'),
            func.count(case([(Order.status == OrderStatus.FAILED, 1)])).label('failed'),
    ...
    async def generate_executive_summary(
        ...
        customer_data = await self.get_customer_analytics(start_date, end_date, tenant_id)
        ...
        return {
            ...
            "top_metrics": {
                "best_selling_product": product_data['top_selling_products'][0] if product_data['top_selling_products'] else None,
                "preferred_payment_method": payment_data['payment_method_distribution'][0] if payment_data['payment_method_distribution'] else None,
                "top_customer": customer_data['top_customers'][0] if customer_data['top_customers'] else None
            },
            "trends": {
                "revenue_trend": revenue_data['daily_revenue_trend'][-7:],  # Last 7 days
                "customer_acquisition": customer_data['customer_acquisition_trend'][-3:]  # Last 3 months
            },
